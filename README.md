...
- 解压完整发布包；不要只复制启动器 EXE。2.0.0 起，官方与自建词库统一存放在 `runtime` 文件夹中。
...
2.0.0 起可以直接在线更新词库和软件，无需 Conda 或 Git 环境。只有维护菜单中的“从客户端提取并重建词库”需要先初始化构建环境。
...
“当前词库”只列出所选区服的词库，双服分别记住选择。可用“导入词库”导入包含 `ARKLOCALIZER_MANIFEST.json` 的完整词库目录，或将它放到 `runtime` 下再刷新。每份词库的 `WORDLIBRARY.json` 保存持久 UUID、名称、区服和可选 Git 来源；改文件夹名称不会改变 ID，重复 ID 会明确报错。原先选择的本地重建词库也会导入并保留选择。
...
游戏字体优先保留原设计：理智等数字、英文装饰文字及能够完整显示中文的原字体不再强制替换。中文字库不完整时，正文使用国服 `NotoSansHans-Medium`，原本使用 Bold/Heavy/Black 字体的标题优先使用国服 `SourceHanSansCN-Heavy`，并保留组件的加粗、字号和材质效果。组件重新显示数字时会恢复原字体。字体不需要安装到 Windows。
...
1.3.1 的“更新连接”支持三种模式：默认“自动”优先使用填写的代理，留空时读取系统代理；代理连接失败后不使用代理重试当前来源。“不使用代理”忽略填写值及系统/环境代理。“仅使用填写的代理”严格使用该地址，失败时提示修正，不切换直连。软件不会修改 Windows 的代理设置。出现 `WinError 10061` 时通常是连接目标拒绝请求，例如本机代理未启动，与目录是否关联 Git 无关。
...
- **软件跟随 Releases**：仅提示更高版本的正式 Release，下载唯一的 `ArknightsLocalizationToolkit*.zip`，核对 GitHub 返回的 SHA-256。1.3.3 起，退出后替换完整 Release 中的启动器、代码和构建工具，以及双服官方词库（包含翻译文本）；官方库按固定 ID 识别，改名后仍能更新。自建词库全部文件保持不变，用户配置、构建环境及缓存保留。代码目录整体替换，移除已废弃的旧模块。已有游戏文件需要再点“安装 / 修复并启动”才会更新。
- **来源可切换**：支持 GitHub 直连或 GitHub 镜像。预填 `https://gh-proxy.com/`，2026-09-06 已验证 API、Raw 和 Release 分段下载可用；镜像地址可随时改成其他 HTTPS 前缀。HTTP(S) 代理也在此弹窗设置。
